...
def compute_v_per_channel(p: Tensor, dim: int | None = None, ternary: bool = False):
...
class LSBQuantizer(Quantizer):
    ...
    @staticmethod
    def quantize_greedy(
        p: Tensor, b: int, dim: int | None = None
    ) -> tuple[Tensor, Tensor]:
        r = p.detach().clone()
        vs = []
    ...
    def quantize_simple_ternary(
        p: Tensor, multiplier: float, dim: int | None = None
    ) -> tuple[Tensor, Tensor]:
